# Conteúdo e copy

## Diretrizes editoriais

- Tom profissional, próximo e direto.
- Posicionamento: tecnologia ampla, com foco em suporte, infraestrutura, operação, administração, atendimento, criação e IA aplicada.
- Não publicar afirmações, resultados, clientes ou certificações além do conteúdo aprovado.

## Conteúdo público atual

| Área | Estado atual |
| --- | --- |
| Perfil | Retrato, nome Diego Garcia, atuação rotativa, cidade e carrossel de dez logos. |
| Sobre | Apresentação profissional, informações pessoais e seis competências centrais. |
| Experiência | Sete vínculos com empresa, período, cargo e descrição expansível. |
| Formação | Ensino médio, Sistemas de Informação, Curso de Direito e timeline expansível de cursos e certificação. Os marcadores anuais encerram os grupos de 2026, 2025, 2024, 2021, 2011 e 2004. |
| Habilidades | Tecnologia e operação; Criação e inovação. |
| Projetos | Cases online RPE6 Strength Academy e Essentia Health, com acesso aos respectivos sites. |
| Serviços | Três categorias expansíveis: TI, criação de conteúdo e soluções web, com itens, valores públicos e observações de cobrança. |
| Informações adicionais | Operação de TI corporativa, escotismo, habilitação amadora e experiência complementar. |
| Contato | WhatsApp com mensagem inicial, e-mail e cidade publicados. |

## Conteúdo aprovado relevante

Na apresentação, o texto rotativo destaca suporte técnico, infraestrutura e redes, operação administrativa, atendimento, gestão de serviços de TI, criação de conteúdo e soluções web. A descrição principal posiciona essas frentes como apoio à rotina, da sustentação técnica à criação de conteúdo e ao desenvolvimento web. O portfólio apresenta competências de suporte técnico, infraestrutura e redes, operação administrativa, atendimento, gestão de serviços de TI e IA aplicada. As habilidades incluem atendimento e incidentes, sistemas e endpoints, identidade e acessos, aplicativos corporativos, redes, ativos, documentação, criação audiovisual, design, LLMs, programação com IA e conteúdo para redes sociais.

WhatsApp e e-mail são links públicos autorizados. O currículo original não é disponibilizado para download porque contém endereço físico.

O case Essentia Health apresenta um site institucional de seis páginas: a página da marca, três páginas do NeuroFlash e duas do Obsidian. O conteúdo aprovado informa 20 dias estimados de produção, investimento total de R$ 1.500 e criação integral dos criativos por Diego Garcia.

O case RPE6 Strength Academy apresenta uma landing page sobre aperfeiçoamento em powerlifting, módulos, método e acesso à plataforma. O conteúdo aprovado informa uma página, cerca de cinco dias de desenvolvimento e investimento total de R$ 700. Os criativos foram fornecidos pelo proprietário.

Os serviços públicos incluem TI, criação de conteúdo e soluções web. Peças de TI, domínio, hospedagem e serviços externos são cobrados separadamente conforme a categoria; projetos web especiais são tratados sob orçamento.

## Restrições

- Não publicar conversas, dados operacionais, contatos, protocolos, arquivos ou detalhes internos relacionados à Sabesp.
- Não publicar endereço físico.
- Não alterar textos, projetos, serviços ou preços sem aprovação.
- Não usar assets sem autorização registrada.
